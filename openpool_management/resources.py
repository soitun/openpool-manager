...
class PaymentResource(ConfigurableResource):
    ...
    def get_client(self):
        """Get a configured Web3Client instance"""
        return Web3Client(self.config)
    ...
    def setup(self):
        ...
        return self.get_client().setup()

    def send_eth(self, amount_wei, to_address):
        """Send ETH to a worker address via client"""
        # Forward the request to the stateful client
        return self.get_client().send_eth(amount_wei, to_address)

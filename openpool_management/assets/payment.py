from dagster import asset, AssetExecutionContext, AssetIn, MetadataValue, Config, Field, job, op, define_asset_job
...
@asset(
    key_prefix=["worker"],
    group_name="tracking",
    description="Track worker payment state and generate payments when threshold is reached",
    ins={"worker_fees": AssetIn(key_prefix=["worker"])},
    partitions_def=multi_partitions,
    io_manager_key="io_manager",
    required_resource_keys={"payment_processor"},
    config_schema={
        "dry_run": Field(bool, default_value=True, is_required=False)
    })
def worker_payments(context, worker_fees):
    """
    Track worker payment state and generate payments when threshold is reached.
    ...
    for eth_address, state in payment_states.items():
        ...
        if unpaid_fees >= payment_threshold_wei:
            ...
            # Track payment amount regardless of dry run status
            # This ensures our reporting is accurate even in dry run mode
            total_payment_amount += unpaid_fees

            # Generate payment only if not in dry run mode
            if not dry_run:
                ...
                try:
                    ...
                    context.log.info(f"[{node_type}/{region}] Executing payment to {eth_address}...")
                    ...
                    context.log.info(
                        f"[{node_type}/{region}] ✓ Generated payment {payment_id} for worker {eth_address}: "
